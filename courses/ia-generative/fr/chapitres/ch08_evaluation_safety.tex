...
% --- hook ---
<< Si vous ne pouvez pas le mesurer, vous ne pouvez pas l'améliorer. >> --- l'aphorisme classique. Pour les modèles génératifs, il faut en ajouter un deuxième : si vous ne testez pas pour le pr\'ejudice, vous l'embarquerez en production. L'évaluation de modèles génératifs est intrinsèquement difficile parce que << correct >> est subjectif, et que les métriques automatiques (BLEU, ROUGE, perplexité) sont des proxys, jamais des vérités. Ce chapitre couvre les métriques quand elles fonctionnent, le red-teaming quand elles ne suffisent pas, et l'alignement (RLHF, DPO) comme tentative de réponse de fond.
