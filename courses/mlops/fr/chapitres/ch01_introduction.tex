...
% --- hook ---
En 2015, une \'equipe de Google publie un article au titre devenu c\'el\`ebre dans la communaut\'e~: \emph{Hidden Technical Debt in Machine Learning Systems}~\cite{sculley2015}. Le diagramme de la page~2 montre une petite bo\^ite intitul\'ee \og{}ML Code\fg{} entour\'ee d'une douzaine d'autres bo\^ites beaucoup plus grandes~: collecte de donn\'ees, validation, surveillance, gestion des features, infrastructure, monitoring. Le mod\`ele lui-m\^eme repr\'esente moins de cinq pour cent du syst\`eme. Le reste est la dette technique cach\'ee dont parle le titre. Cette d\'ecouverte explique pourquoi tant de projets de machine learning, brillants sur un notebook Jupyter, meurent silencieusement avant d'atteindre la production~: le mod\`ele \'etait pr\^et, mais le syst\`eme qui devait l'h\'eberger n'existait pas. Le MLOps est la discipline qui prend en charge ces douze autres bo\^ites. Ce chapitre d\'efinit le terme, d\'eroule les six phases du cycle de vie d'un mod\`ele en production, et pr\'esente les pi\`eges classiques (data drift, biais d'entra\^inement-servi, mod\`eles fant\^omes) qui font \'echouer les d\'eploiements quand on les ignore.
